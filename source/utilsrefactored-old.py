...
def build_root_iverted(model_name, percent, withHUSHDP = True):

  if model_name == 'mnist-basic':
    if percent == 0.5:
      if withHUSHDP:
        with tf.Graph().as_default():
          root_model = tf.keras.models.clone_model(basic_mnist_hush_dp_model)
          root_model.pop()
          root_model.pop()
          root_model.pop()
          # root_model = basic_mnist_hush_dp_model.layers[:2]
          # root_model = tf.keras.models.Sequential([
          #     tf.keras.layers.Flatten(input_shape=(28, 28)),
          #     tf.keras.layers.Dense(128, activation='sigmoid'),
          #     tf.keras.layers.Dense(200, activation='sigmoid')
          #     ])
          # print(basic_mnist_hush_dp_model.get_weights()) 
      else:
        root_model = tf.keras.models.Sequential([
            tf.keras.layers.Flatten(input_shape=(28, 28)),
            tf.keras.layers.Dense(128, activation='sigmoid'),
...
def get_optimizer(choice, model, trunk_count):
...
def build_segment_models_inverted(model_name, segment_range, root_model, percent, withHUSHDP = True):
  flavor_choice = "normal"
  if model_name == 'mnist-basic':
    if percent == 0.5:
      ...
      for i in segment_range:
        ...
        with tf.Graph().as_default():
          hushdp_model = tf.keras.models.clone_model(basic_mnist_hush_dp_model)
          for layer in hushdp_model.layers[:2]:
            layer.trainable = False

          model_segment_full[i] = hushdp_model
        ...
        #   ])
          ...
          ############################################################################
          # opt = get_optimizer(flavor_choice, model_segment_full[i], len(root_model.layers))
...
def get_latency_acc_dict(model_name, ds_name, seg_count, experiment_code, percent = 0.5):

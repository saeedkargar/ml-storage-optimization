...
temp_model_obj = mn.MobileNet()
mobilenet_benchmark_model = temp_model_obj.model

def printdict(d):
  print(json.dumps(str(d), indent=4)) # or d.item() instead of str(d)

# create and train level 1 components of the model
def build_root_iverted(model_name, percent):
  ...
  if model_name == 'mnist-basic':
    ...
    # it the root is empty or contains all layers
    if layer_count < 1 or layer_count == len(basic_mnist_benchmark_model.layers): 
    ...
    temp_model = tf.keras.models.clone_model(mobilenet_benchmark_model)
    layer_count = ceil(len(temp_model.layers)*percent)
    print('the root layer count is : ', layer_count)
    if layer_count < 1  or layer_count == len(temp_model.layers): 
      print('there was a problem building the root model')
      return None
    root_model = tf.keras.models.Sequential(temp_model.layers[:layer_count])
...
def build_SISA_models(model_name, shard_range):
  if model_name == 'mnist-basic':
    ...
    #SISA_constituent_models items represent the entirity of the model
    SISA_constituent_models = list(shard_range)
    ...
    for i in shard_range:
      ...
      SISA_constituent_models[i] = tf.keras.models.Sequential([
        root_model,
        part_model
        ])

      # SISA_constituent_models[i].summary()
      SISA_constituent_models[i]._name = "model_full_segment"+str(i)
      SISA_constituent_models[i].compile(
          optimizer=tf.keras.optimizers.Adam(0.001),
          loss=tf.keras.losses.SparseCategoricalCrossentropy(from_logits=False),
          metrics=[tf.keras.metrics.SparseCategoricalAccuracy()]
...
def saveresultsfile(argumnets, model, results, approach):
  ...
  results_Path = path + "/results/" + result_json_name
...
def get_latency_acc_dict(model_name, ds_name, segment_list, experiment_code, percent = 0.5):
  ...
  for segment_count in segment_list: 
      ...
      ds_train = load_data(segment_count, segment_range, ds_name)
      ...
      if (model_name in ['vgg', 'resnet', 'mobilenet'] and ds_name in ['cifar10']) or (model_name in ['mnist-basic'] and ds_name in ['mnist']):
  ...
  return accuracy_latency_dict, ds_info, ds_test


def get_latency_acc_dict_SISA(model_name, ds_name, segment_list, experiment_code, percent = 0.5):
  accuracy_latency_dict = dict() #dictionary for collecting accuraccy and loss of different segment models

  for shard_count in segment_list: 
      if experiment_code == "segment-count":
        mainKey = str(shard_count)
      elif experiment_code == "root-size":
        mainKey = str(percent)
      ...
      ds_train = load_data(shard_count, shard_range, ds_name)
...
def train_segments(segment_range, SISA_models, ds_train, ds_test):
  histories = list()
  c = 0
  ...
  for i in segment_range:
    c += 1
    print('training SISA_models[', i, '] using data segment', i)
    
    start_our_retraining = t()
    histories.append(SISA_models[i].fit(
        ds_train[i],
        epochs=20,
  ...
  return SISA_models, histories, latency_our_retraining
...
def build_benchmark(model_name, ds_name, ds_info, withDP):
  ...
  if model_name == 'mnist-basic' and ds_name == 'mnist':
  ...
  elif model_name == 'mobilenet' and ds_name == 'cifar10':
    learning_rate = 0.25 #, 'Learning rate for training')
    noise_multiplier = 0.1 #'Ratio of the standard deviation to the clipping norm')
    l2_norm_clip = 1.5 #, 'Clipping norm')
    microbatches = batch_size #, 'Number of microbatches '(must evenly divide batch_size)')
    ...
    benchmark_model = tf.keras.models.clone_model(mobilenet_benchmark_model)
...
def get_latency_acc_dict_benchmark(model_name, ds_name, ds_info, ds_test, differential_privacy):
  benchmark_model, benchmark_ds_train = build_benchmark(model_name, ds_name, ds_info, differential_privacy)
